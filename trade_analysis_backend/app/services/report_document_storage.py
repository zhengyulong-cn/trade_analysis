...
class ReportDocumentStorageService:
    ...
    async def save_and_extract(self, file: UploadFile) -> dict[str, str | int]:
        ...
        content_type = file.content_type or self._guess_content_type_by_suffix(suffix)

        if suffix not in self.allowed_suffixes and content_type not in self.allowed_content_types:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Unsupported report file type, only pdf and txt are supported",
            )
        ...
        extension = self._resolve_extension(original_name, content_type)
        ...
        raw_text = self._extract_text(content=content, suffix=extension)
    ...
    def _extract_text(self, content: bytes, suffix: str) -> str:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Unsupported report file type for text extraction",
        )
    ...
    def _extract_txt_text(self, content: bytes) -> str:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Unsupported txt encoding, expected utf-8 or gb18030 compatible text",
        )
    ...
    def _resolve_extension(self, filename: str, content_type: str) -> str:
        ...
        if suffix in self.allowed_suffixes:
            return suffix
        return self._guess_extension_by_content_type(content_type)

    def _guess_extension_by_content_type(self, content_type: str) -> str:
        mapping = {
            "application/pdf": ".pdf",
            "text/plain": ".txt",
        }
    ...
    def _guess_content_type_by_suffix(self, suffix: str) -> str:
        mapping = {
            ".pdf": "application/pdf",
            ".txt": "text/plain",
        }
