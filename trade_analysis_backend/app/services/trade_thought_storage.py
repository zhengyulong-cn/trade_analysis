...
class TradeThoughtStorageService:
    allowed_content_types = {"image/jpeg", "image/png", "image/webp", "image/gif"}
    max_file_size = 10 * 1024 * 1024

    def __init__(self):
        self.storage_root = Path(settings.storage_root)
        self.trade_thought_dir = self.storage_root / "trade_thoughts"
        self.trade_thought_dir.mkdir(parents=True, exist_ok=True)
    ...
    async def save_image(self, file: UploadFile) -> dict[str, str | int]:
        ...
        relative_dir = Path("trade_thoughts")
