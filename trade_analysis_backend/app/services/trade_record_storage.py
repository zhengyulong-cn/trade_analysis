...
class TradeRecordStorageService:
    ...
    def __init__(self):
        ...
        self.trade_record_dir = Path(settings.trade_record_storage_dir)
        self.trade_record_dir.mkdir(parents=True, exist_ok=True)

    async def save_screenshot(self, file: UploadFile) -> dict[str, str | int]:
        ...
        relative_dir = Path("trade_records")
        ...
        if size > self.max_file_size:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Screenshot file exceeds 10 MB limit",
            )
